...
An application interceptor is added to the `OkHttpClient` created by autoconfiguration which resolves the scheme, host and port from Spring Cloud LoadBalancer and rewrites the url.
