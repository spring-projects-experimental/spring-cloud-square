...
We provide tracing support via Spring Cloud Sleuth integration. If you add `spring-cloud-starter-sleuth` to your project setup, tracing will be provided out of the box, either using the `WebClient` tracing integration provided Spring Cloud Sleuth or via `OkHttpClient` tracing integration provided by the `spring-cloud-square-okhttp` module.

TIP: You can disable OkHttp tracing support by setting the value of ``"``okhttp.tracing.enabled` to `false`.
